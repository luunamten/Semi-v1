...
public class SharedPrefs {
    ...
    private SharedPrefs() {
        mSharedPreferences = MyApp.getContext().getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);
    }
    ...
    public <T> T get(String key, Class<T> anonymousClass) {
        if (anonymousClass == String.class) {
            ...
            return (T) MyApp.getInstancee().getGSon().fromJson(mSharedPreferences.getString(key, ""), anonymousClass);
        }
    }
    @SuppressWarnings("unchecked")
    ...
    public <T> void put(String key, T data) {
        ...
        if (data instanceof String) {
            ...
            editor.putString(key, MyApp.getInstancee().getGSon().toJson(data));
        }
        editor.apply();
    }
